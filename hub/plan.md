...
O Hub será o coração do sistema, roteando dados e controlando o estado da transmissão.
...
1.2. Módulo de Roteamento e Estado (WebSockets/TCP):
...
Ação (Status de Espera): Implementar um controle de estado (hasSourceConnected). Se nenhuma Fonte estiver enviando vídeo, o Hub emite uma mensagem periódica de { status: 'waiting' } para os Clientes, em vez de enviar dados de vídeo vazios ou quebrar a conexão.
...
3.3. Recepção, Exibição de Vídeo e UX de Espera:

Ação: O aplicativo abre uma conexão TCP/WebSocket no IP descoberto.

Ação (Tratamento de Estado): O app escuta os pacotes recebidos. Se receber { status: 'waiting' }, a interface renderiza uma tela nativa de "Aguardando Transmissão..." ou uma logo girando, economizando processamento.

Ação (Renderização): Quando os pacotes binários começam a chegar, a interface esconde a tela de espera e utiliza um componente de Video Player nativo (react-native-video ou expo-av) para renderizar os frames do stream na tela.
